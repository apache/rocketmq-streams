...
public class RocketmqDispatcher<T> implements IDispatcher<T> {
                    ...
                    this.messageListener.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
                        try {
                            for (MessageExt ext : msgs) {
                                ...
                                LOGGER.info("[{}] Received_Message({})_Local({})", this.instanceName, receivedMessage, String.join(",", localCache));
                                ...
                                if ("stop".equals(tags)) {
                                    ...
                                    if (!needStart.isEmpty()) {
                                        //启动前，重新加载一下内存中的任务实例
                                        this.dispatcherCallback.list(Lists.newArrayList(consumerIdList));
    ...
    public void doDispatch(String requestId, MQClientInstance mqClientInstance, int depth) throws Exception {
        if (depth > 2) {
            return;
        }
        ...
        if (consumerList != null && !consumerList.isEmpty()) {
            ...
            if (isMaster) {
                if (!consumerIdList.isEmpty()) {
                    TreeSet<String> tasks = Sets.newTreeSet(this.dispatcherCallback.list(Lists.newArrayList(consumerIdList)));//此逻辑必须放在选主之前，因为所有实例都需要加载任务
    ...
    @Override public void wakeUp() throws Exception {
    ...
    @Override public List<String> getClients() throws Exception {
        return Lists.newArrayList(this.consumerIdList);
    }
