...
public abstract class BasedLeaseImpl implements ILeaseService {
    private static final Logger LOGGER = LoggerFactory.getLogger(BasedLeaseImpl.class);
     ...
     * @return
     */
    @Override public boolean hasLease(String name) {
        // 内存中没有租约信息则表示 没有租约
    ...
    @Override public void startLeaseTask(final String name) {
        startLeaseTask(name, this.leaseTerm, null);
    }

    @Override public void startLeaseTask(final String name, ILeaseGetCallback callback) {
        startLeaseTask(name, this.leaseTerm, callback);
    }

    @Override public void startLeaseTask(final String name, int leaseTerm, ILeaseGetCallback callback) {
    ...
    protected void startLeaseTask(final String name, Runnable runnable, int scheduleTime, boolean startNow) {
        ...
        if (isStartLease.compareAndSet(false, true)) {
            ...
            ScheduleFactory.getInstance().execute(BasedLeaseImpl.class.getName() + "-base_lease_schedule", runnable, 0, scheduleTime, TimeUnit.SECONDS);
        }
    }

    @Override public void stopLeaseTask(String name) {
        AtomicBoolean isStartLease = startLeaseMap.get(name);//多次调用，只启动一次定时任务
        if (isStartLease == null) {
            synchronized (this) {
                isStartLease = startLeaseMap.get(name);
        ...
        if (isStartLease == null || isStartLease.compareAndSet(true, false)) {
            ScheduleFactory.getInstance().cancel(BasedLeaseImpl.class.getName() + "-base_lease_schedule");
            //释放
            releaseLeaseInfo(name);
    ...
    protected boolean tryGetLease(String name, Date time) {
        ...
        if (validateLeaseInfo == null) {// 这里有两种情况 1 数据库里面没有租约信息 2 数据库里面有租约信息但是已经过期
            ...
            if (leaseUserIp.equals(getSelfUser())) {
                ...
                if (hasUpdate) {
                    LOGGER.info("LeaseServiceImpl机器重启情况，当前用户有租约信息，并且更新数据库成功，租约信息为 name :" + validateLeaseInfo.getLeaseName() + " ip : " + validateLeaseInfo.getLeaseUserIp() + " 到期时间 : " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(validateLeaseInfo.getLeaseEndDate()));
                    return true;
                } else {
    ...
    protected void releaseLeaseInfo(String name) {
        // LOG.info("LeaseServiceImpl unlock,name:" + name);
        LeaseInfo validateLeaseInfo = queryValidateLease(name);
        if (validateLeaseInfo == null) {
            LOGGER.warn("LeaseServiceImpl unlock,validateLeaseInfo is null,lockerName:" + name);
        }
        ...
        @Override public void run() {
            try {
                ...
                if (leaseDate != null) {
                    ...
                    LOGGER.info("LeaseServiceImpl, name: " + name + " " + getSelfUser() + " 获取租约成功, 租约到期时间为 " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(leaseDate));
                } else {
