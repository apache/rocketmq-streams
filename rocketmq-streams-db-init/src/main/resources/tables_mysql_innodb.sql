...
(
    ...
    `max_offset` varchar(256) NOT NULL,
    `is_max_offset_long` int(1) DEFAULT NULL,
...
(
    ...
    `group_by_field_name` varchar(128) DEFAULT NULL COMMENT '分组字段',
    PRIMARY KEY(`id`),
...
(
    ...
    `send_task_msg` longtext NOT NULL,
...
(
    ...
    `can_clear_resource` tinyint(4) DEFAULT NULL COMMENT '是否清理资源',
    PRIMARY KEY(`id`),
...
(
    ...
    `message_body` longtext,
    `msg_key` varchar(400) DEFAULT NULL,
    `window_instance_id` varchar(20) DEFAULT NULL,
    `partition` varchar(200) DEFAULT NULL,
    `partition_num` bigint(20) DEFAULT NULL,
    `window_instance_partition_id` varchar(200) DEFAULT NULL,
    `version` varchar(64) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `dipper_dispatcher_kv_config`
(
    ...
    UNIQUE KEY `uk_message_id_unique`(`namespace`,`config_key`),
    KEY `idx_namespace_index`(`namespace`),
    KEY `idx_key_index`(`config_key`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;