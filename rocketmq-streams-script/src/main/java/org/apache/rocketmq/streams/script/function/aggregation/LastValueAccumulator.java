...
public class LastValueAccumulator<T> implements IAccumulator<T, LastValueAccumulator.LastValue> {
    ...
    @Override
    public T getValue(LastValue accumulator) {
        return (T) accumulator.value;
    }

    @Override
    public void accumulate(LastValue accumulator, Object... parameters) {
    ...
    @Override
    public void merge(LastValue accumulator, Iterable<LastValue> its) {
        Iterator<LastValue> it = its.iterator();
        while (it.hasNext()) {
            LastValue next = it.next();
