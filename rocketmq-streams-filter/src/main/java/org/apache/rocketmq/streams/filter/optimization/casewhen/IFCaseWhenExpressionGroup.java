...
public class IFCaseWhenExpressionGroup implements IScriptExpression {
    ...
    @Override public Object executeExpression(IMessage message, FunctionContext context) {
        if(cache==null){
            synchronized (this){
                if(cache==null){
                    cache=new FingerprintCache(1000000);
                }
            }
        }
    ...
    public static void main(String[] args) {
        FunctionScript functionScript=new FunctionScript(FileUtil.loadFileContent("/Users/yuanxiaodong/Downloads/script_test.txt"));
        functionScript.init();
        functionScript.doProcessAfterRefreshConfigurable(ConfigurableComponent.getInstance("namespace"));
