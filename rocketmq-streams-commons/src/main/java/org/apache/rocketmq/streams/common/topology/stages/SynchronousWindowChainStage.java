...
public class SynchronousWindowChainStage<T extends IMessage> extends AbstractStatelessChainStage {

    private static final long serialVersionUID = -6592591896560866562L;

    @ConfigurableReference protected IWindow window;

    public SynchronousWindowChainStage() {
        super.entityName = "window";
    }

    @Override
    protected IMessage handleMessage(IMessage message, AbstractContext context) {
        if (window instanceof ISynchronousWindow) {
            ISynchronousWindow notNeedShuffle = (ISynchronousWindow) window;
            notNeedShuffle.accumulateDirectly(message, context);
        }
