...
public class FileSource extends AbstractSource {
    ...
    protected boolean initConfigurable() {
        ...
        if (file.exists() && file.isDirectory()) {
            if (executorService == null) {
                executorService = ThreadPoolFactory.createFixedThreadPool(maxThread, FileSource.class.getName() + "-" + getName());
            }
        }
        if (file.exists() && !file.isDirectory()) {
            if (executorService == null) {
                executorService = ThreadPoolFactory.createFixedThreadPool(1, FileSource.class.getName() + "-" + getName());
            }
        }
     ...
     */
    public LinkedBlockingQueue<FileIterator> createIteratorList() {
        ...
        try {
            ...
            for (File subFile : files) {
                if (namePattern == null) {
                    ...
                    if (StringUtil.matchRegex(subFile.getAbsolutePath(), namePattern)) {
                        iterators.add(new FileIterator(subFile));
                    }
                }
            }
