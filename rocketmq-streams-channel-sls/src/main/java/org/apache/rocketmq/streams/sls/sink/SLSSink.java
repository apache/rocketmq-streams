...
public class SLSSink extends AbstractSupportShuffleSink {
    ...
    private boolean initSts() {
        if (isSts) {
            ...
            if (this.stsRefreshPool == null) {
                this.stsRefreshPool = ThreadPoolFactory.createScheduledThreadPool(1, SLSSink.class.getName() + "-" + getConfigureName());
            }
    ...
    protected void startRefreshAsync() {
        if (isSts) {
            ...
            stsRefreshPool.scheduleWithFixedDelay(stsRefreshTask, stsService.getRefreshTimeSecond(), stsService.getRefreshTimeSecond(), TimeUnit.SECONDS);
        }
    }
