...
public class SLSUtil {
     ...
     *
     * @param source
     * @param shardId2Cursor
     * @param client
     * @param project
     * @param logStore
     * @return
     */
    public static List<SplitProgress> getSplitProgress(ISource source, Map<String, String> shardId2Cursor, Client client, String project, String logStore) {
        try {
            List<ISplit<?, ?>> splits = source.fetchAllSplits();
            List<SplitProgress> splitProgresses = new ArrayList<>();
            if (splits == null) {
                return null;
            }
            Map<String, ISplit> shardId2Splits = new HashMap<>();
            for (ISplit<?, ?> split : splits) {
                Shard shard = (Shard) split.getQueue();
                shardId2Splits.put(shard.getShardId() + "", split);
            }
            for (String shardId : shardId2Cursor.keySet()) {
                ISplit split = shardId2Splits.get(shardId);
                Shard shard = (Shard) split.getQueue();
                String cursor = shardId2Cursor.get(shardId);
                GetCursorTimeResponse response = client.GetCursorTime(project, logStore, shard.getShardId(), cursor);
                if (response != null) {
                    SplitProgress splitProgress = new SplitProgress(split.getQueueId(), response.GetCursorTime(), true);
                    splitProgresses.add(splitProgress);
                }
            }
            return splitProgresses;
        } catch (Exception e) {
            throw new RuntimeException("Error while listing shards", e);
        }
    }
}
