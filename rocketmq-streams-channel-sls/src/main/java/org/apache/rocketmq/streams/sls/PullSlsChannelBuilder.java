...
import org.apache.rocketmq.streams.sls.sink.SLSSink;
import org.apache.rocketmq.streams.sls.source.SLSPullSource;

@AutoService(IChannelBuilder.class) @ServiceName(value = PullSlsChannelBuilder.TYPE, aliasName = "SLSPullSource") public class PullSlsChannelBuilder extends AbstractSupportShuffleChannelBuilder {
    public static final String TYPE = "sls_pull";
    ...
    @Override public ISource<?> createSource(String namespace, String name, Properties properties, MetaData metaData) {
        return (ISource<?>) ConfigurableUtil.create(SLSPullSource.class.getName(), namespace, name, createFormatProperty(properties), null);
    }
    ...
    @Override public ISink<?> createSink(String namespace, String name, Properties properties, MetaData metaData) {
        return (ISink<?>) ConfigurableUtil.create(SLSSink.class.getName(), namespace, name, createFormatProperty(properties), null);
    }
    ...
    protected JSONObject createFormatProperty(Properties properties) {
        ...
        for (Object object : properties.keySet()) {
            ...
            //properties.getProperty 只能获取字符串的值
    ...
    @Override public ISink<?> createBySource(ISource<?> pipelineSource) {
        SLSPullSource source = (SLSPullSource) pipelineSource;
        return new SLSSink(source.getEndPoint(), source.getProject(), source.getLogStore(), source.getAccessId(), source.getAccessKey());
    }
}
