...
public abstract class AbstractIntelligenceCache extends BasedConfigurable {
    ...
    @ConfigurableReference protected IDBDriver outputDataSource;
    ...
    public IntValueKV startLoadData(String sql, IDBDriver resource) {
        try {
            ...
            LOGGER.info("[{}] {} load data finish, load data line  size is {}", getName(), getClass().getSimpleName(), intValueKV.getSize());
            return intValueKV;
        } catch (Exception e) {
            LOGGER.error("[{}] failed loading intelligence data!", getName(), e);
            return new IntValueKV(0) {
                @Override
                public Integer get(String key) {
                    return null;
    ...
    public void startIntelligenceInner() {
        ...
        if (hasInit.compareAndSet(false, true)) {
            ...
            ScheduleFactory.getInstance().execute(getNameSpace() + "-" + getName() + "-intelligence_schedule", new Runnable() {
                @Override
                public void run() {
    ...
    public boolean dbInit() {
        try {
            int successCode = 200;
            String region = getConfiguration().getProperty(ConfigurationKey.INTELLIGENCE_REGION);
            String ak = getConfiguration().getProperty(ConfigurationKey.INTELLIGENCE_AK);
            String sk = getConfiguration().getProperty(ConfigurationKey.INTELLIGENCE_SK);
            String endpoint = getConfiguration().getProperty(ConfigurationKey.INTELLIGENCE_TIP_DB_ENDPOINT);
                ...
                if (successCode == code) {
                    ...
                    if (StringUtils.isNotBlank(content)) {
                        ...
                        if (dbInfo != null) {
                            ...
                            dataSource.init();
                            LOGGER.debug("[{}] succeed in getting db information from tip service!", getName());
                            return true;
                        }
                    }
                }
            }
            LOGGER.error("[{}] failed in getting db information from tip service!", getName());
            return false;
        } catch (Exception e) {
            LOGGER.error("[{}] failed in getting db information from tip service!", getName(), e);
            return false;
        }
    }
    ...
    public IDBDriver getOutputDataSource() {
        return outputDataSource;
    }

    public void setOutputDataSource(IDBDriver outputDataSource) {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    LOGGER.info("[{}] {}, finished count is {} the total count is {}, the progress is {}%", getName(), cache.getClass().getSimpleName(), count, totalSize, String.format("%.2f", progress));
                    if (size < batchSize) {
                        if (size > 0) {
